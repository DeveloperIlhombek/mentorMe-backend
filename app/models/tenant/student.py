...
class Student(Base):
    ...
    # Sarafan: o'quvchi qanday yo'l bilan kelgani (teacher | social | friend | ad | other)
    referral_source:         Mapped[Optional[str]]       = mapped_column(String(30), nullable=True)
    referred_by_teacher_id:  Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("teachers.id", ondelete="SET NULL"), nullable=True)
    # Churn: o'quvchi ketish sababi (teacher | price | schedule | location | other)
    leave_reason:            Mapped[Optional[str]]       = mapped_column(String(50), nullable=True)
    churn_teacher_id:        Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("teachers.id", ondelete="SET NULL"), nullable=True)
    # O'zlashtirish kunlari: [X, Y] — oyning qaysi kunlarida progress kiritilishi kerak
    progress_dates:          Mapped[list]                = mapped_column(JSONB, default=list, server_default="'[]'::jsonb", nullable=False)
