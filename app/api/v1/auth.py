...
async def _discover_tenant_by_tg(telegram_id: int) -> Optional[tuple[User, str]]:
    """
    Barcha aktiv tenantlardan telegram_id'ga ega user'ni qidirish.
    Stale `persistedTenantSlug` (frontend localStorage) muammosini hal qiladi:
    foydalanuvchi avval bir tenantga, keyin boshqasiga registratsiya qilgan
    bo'lsa, brauzer eski slug yuborsa ham — biz to'g'risini topamiz.

    Qaytaradi: (user, tenant_slug) yoki None.
    """
    async with AsyncSessionLocal() as public_session:
        slugs = (await public_session.execute(
            select(Tenant.slug).where(Tenant.is_active == True)  # noqa: E712
        )).scalars().all()

    for slug in slugs:
        try:
            u = await _get_tenant_user_by_tg(slug, telegram_id)
            if u:
                return u, slug
        except Exception:
            continue
...
async def telegram_login(
    ...
    matched_slug = data.tenant_slug

    if not user:
        # Auto-discovery: stale `persistedTenantSlug` muammosini hal qilish.
        # Foydalanuvchi boshqa tenantda ro'yxatdan o'tgan bo'lishi mumkin.
        discovered = await _discover_tenant_by_tg(telegram_id)
        if discovered:
            user, matched_slug = discovered
        else:
            raise HTTPException(
                status_code=403,
                detail={
